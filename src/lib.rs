...
use async_openai::{
  types::{
      CreateAssistantRequest, CreateMessageRequest, CreateThreadRequestArgs, CreateRunRequest, RunStatus, MessageContent
  },
  Client,
};
use anyhow::Result;
...
pub async fn run(address: Address, prompt: String, settings: Settings) -> Result<()>{
  ...
  let assistant_object = assistants.data.into_iter().find(|assistant| assistant.name == Some("castGPT-6".to_string()));

  let assistant_object = match assistant_object {
    Some(assistant) => assistant,
    None => {
      let assistant_request = CreateAssistantRequest {
        name: Some("castGPT-6".to_string()),
  ...
  let abi = get_abi(address.to_string(), ethers_core::types::Chain::Mainnet).await.unwrap();

  let create_message_request = CreateMessageRequest {
      role: "user".to_string(),
      content: format!("The contract address is: {}. Rpc url is: {}. Abi is: {}. My query is {}",  address, settings.rpc_url, abi, prompt),
      file_ids: None,
      metadata: None,
  };
  ...
  let create_run_request = CreateRunRequest {
      ...
      model: Some("gpt-4-1106-preview".to_string()),
      instructions: Some("Generate the cast command. Do not provide any other text or special characters besides the command itself. Do not include an abi option flag. Only use the function signature format specified in the examples provided.".to_string()),
      tools: None,
      metadata: None,
  };
  ...
  let message = client.threads().messages(&run_status.thread_id).list(&query).await?;
  ...
  let cast_cmd = match assistant_res {
      MessageContent::Text(text) => {
          Ok(text.text.value.trim().to_string().clone())
      },
      _ => {
